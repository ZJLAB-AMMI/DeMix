...
def get_sgd(params, conf):
    return torch.optim.SGD(params, conf.lr, momentum=conf.momentum, weight_decay=conf.weight_decay, nesterov=True)
...
def get_multisteplr(optim, conf):
    return torch.optim.lr_scheduler.MultiStepLR(optim, milestones=conf.lrstep, gamma=conf.lrgamma, last_epoch=-1)
